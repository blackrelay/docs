...
Black Relay mostly helps with the first job. It gives builders an indexed, source-aware read layer so a tool does not need to run its own full backfill before it can show names, systems, gates, characters or killmails.
